...
当前版本:**0.4.2**(Windows x64;NSIS 安装版 + 便携版,见 `release/`)。界面语言:中文 / English(默认跟随系统,托盘菜单可切换)。安装版可自动从 GitHub Releases 升级(见「应用自动升级」)。
...
- 传输中断会自动重试:整条下载源列表最多重跑 4 轮,每轮都**从断点续传**,
  110 MB 的安装包被切断也不会丢失进度(窗口会提示「正在续传重试」);
...
| 0.4.1 | 下载前先探测 github.com 直连是否可用(被墙时直接走加速代理,省掉每次 15 秒连接超时);Release 正文模板(升级窗口会显示) |
| 0.4.2 | 传输中断自动重试整条下载源列表并从断点续传(最多 4 轮),升级窗口显示「正在续传重试」——针对国内网络下载 ~110 MB 安装包被中途切断的问题 |
